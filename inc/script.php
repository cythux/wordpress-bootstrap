...
function bootstrap_scripts() {
  ...
  wp_register_script('bootstrap_scripts', get_template_directory_uri() . '/js/scripts.min.js', false, '2a3e700c4c6e3d70a95b00241a845695', true);
  wp_enqueue_script('modernizr');
  wp_enqueue_script('jquery');
  wp_enqueue_script('bootstrap_scripts');
}
add_action('wp_enqueue_scripts', 'bootstrap_scripts', 100);
...
function bootstrap_jquery_local_fallback($src, $handle) {
  ...
  if ($add_jquery_fallback) {
    echo '<script>window.jQuery || document.write(\'<script src="' . get_template_directory_uri() . '/js/vendor/jquery-1.10.2.min.js"><\/script>\')</script>' . "\n";
...
function bootstrap_google_analytics() { ?>
<script>
...
if (GOOGLE_ANALYTICS_ID) {
  add_action('wp_footer', 'bootstrap_google_analytics', 20);
}
